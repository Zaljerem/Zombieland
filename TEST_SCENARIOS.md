...
- `C1-stack-container` is covered by live evidence from 2026-05-29 and rerun evidence from 2026-05-30. `zombieland/contamination_stack_absorb_contract` passed with weighted merge `0.2 x6 + 0.8 x4 => 0.44 x10`. `zombieland/contamination_carry_tracker_contract` passed on the current build by putting contaminated `Steel x20` into a drafted colonist's hands, advancing to the carry cadence tick, and changing pawn/item contamination to `0.060200002`/`0.6398` while the item stayed held. `zombieland/contamination_ingestion_contract` passed by ingesting one meal from a contaminated five-meal stack, changing eater contamination to `0.09600001` with matching need/hediff severity and leaving the meal stack at `0.403999984`. `rimbridge/list_logs minimumLevel=warning` returned no entries.
...
- A 2026-05-30 C3 generation-handoffs run from `EMPTY` added `zombieland/contamination_generation_handoffs_contract` for reward/loot, trade, and mech-cluster products after decompiling `ThingSetMaker.Generate`, `TradeDeal.AddAllTradeables`, and `MechClusterUtility.SpawnCluster`. The installed targets were `ThingSetMaker.Generate(ThingSetMakerParams)`, private `TradeDeal.AddAllTradeables()`, and the 1.6 `MechClusterUtility.SpawnCluster(IntVec3, Map, MechClusterSketch, Boolean, Boolean, String)` signature. With random contamination chances forced to `1`, a generated `ComponentIndustrial x3` and `Steel x25` received contamination `0.225987986` and `0.151718691`; synthetic trader goods appeared in real `TradeDeal` trader tradeables and received nonzero contamination; and a real mech-cluster sketch spawned `Thing_Barricade6903` at `(172,100)` with uniform contamination `4.659071e-6`. `rimbridge/list_logs minimumLevel=warning` returned no entries.
