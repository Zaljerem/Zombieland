...
Completion:
- Partially covered by focused contracts and the 2026-05-30 medical/corpse/filter patch runs, including direct zombie corpse creation through `ThingMaker.MakeThing`. Remaining workflow proof: save-load during corpse conversion countdown, infection alert list behavior, and cure/double-tap/extract/medical UI in one player-facing map session.
