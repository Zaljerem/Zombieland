...
- Live `EMPTY.rws` fire/tar sub-contract pass: rain vulnerability stayed vanilla for human fire and disabled-setting zombie fires but was intermittently suppressed for normal/spitter/blob zombies with `zombiesBurnLonger=true`; sampled `Fire.DoFireDamage` injury was lower for normal/spitter/blob zombies; burning `TarSlime` spread fire to adjacent tar and emitted `TarSmoke`; `FireWatcher.UpdateObservations` counted map plus human fire while excluding attached normal/spitter/blob zombie fires. The warning-or-higher log check was clean.
...
- Focused smart-melee coverage was added on 2026-05-30 after decompiling `Verb_MeleeAttack.TryCastShot` and `Verb.TryStartCastOn`. `zombieland/area_workflow_state setupFixture:true actionMode:smart-melee` from `EMPTY.rws` returned `success=true` and recorded the installed Zombieland prefix on `Verb_MeleeAttack.TryCastShot`. The verifier calls the patched `TryCastShot` method directly for branch-result cases because `Verb.TryStartCastOn` only reports whether the cast started. A normal zombie `ZombieBite` against a melee-20 armed colonist with `safeMeleeLimit=1` returned false and caused no injury; a chainsaw-equipped colonist melee verb returned false and caused no injury; an armed colonist attacking a roped zombie returned false and killed the zombie; and an ordinary armed colonist attacking a normal zombie had the prefix continue to vanilla with `Pawn_MeleeVerbs.TryMeleeAttack=true`. `rimbridge/list_logs minimumLevel=warning` returned no entries.
...
- Focused zombie-stat coverage was added on 2026-05-30 after decompiling `StatExtension.GetStatValue(Thing, StatDef, bool, int)`. `zombieland/area_workflow_state setupFixture:true actionMode:zombie-stats` from `EMPTY.rws` returned `success=true` and recorded the installed Zombieland prefix on the real static extension method. The probe temporarily set `moveSpeedIdle=0.25`, `moveSpeedTracking=0.75`, `spitterThreat=2.25`, `ZombieTicker.PercentTicking=1`, and `TimeSpeed.Normal`, then restored the prior state. Real `GetStatValue` calls proved ordinary human `MoveSpeed` passes through, idle Fat zombie speed is `0.805`, tracking Thin is `2.76`, former-map-pawn Hulk is `5.52`, tanky is `0.0184`, roped health-downed crawler is `0.4`, melee hit/dodge/pain-shock branches match former/downed/tanky/tracking/albino/body-type expectations, tanky comfortable temperature is `-999..999`, and spitter `IncomingDamageFactor` is `3.75`. `SmokeSensitivity` and `Suppressability` were absent in the active def set, so their ignored-stat branch is source-covered only. `rimbridge/list_logs minimumLevel=warning` returned no entries.
