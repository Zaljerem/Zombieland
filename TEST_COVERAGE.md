...
- Live ticking/capacity/downing/projectile/collision evidence: `zombieland/zombie_ticking_budget_contract` from `EMPTY.rws` proved 20 spawned live zombies under `PercentTicking=0.25` produced a five-zombie ticking subset, all selected zombies were live, exactly five production ticks were counted, and throttled move speed stayed at 4x normal speed. An anesthetized low-consciousness zombie (`Consciousness=0.01`) still returned `CanBeAwake=true` while the anesthetized human control at the same consciousness returned false. The same contract seeded 197 pheromone cells around a human, invoked `Pawn_HealthTracker.MakeDowned`, and proved the human became health-downed with all seeded cells cleared, while spitter and blob remained not health-downed. It also proved pending zombie spawns force `TickManager.NothingHappeningInGame=false`, a human `Projectile.Launch` bumped 341 pheromone cells while a `ZombieSpitter` launch bumped none, and direct zombie path-collision/tweener probes returned false/zero. `rimbridge/list_logs minimumLevel=warning` returned no entries.
...
Current gap:
- Many core primitives are covered, including ambient temperature normalization for Zombieland pawns/corpses versus ordinary vanilla things, combat-log suppression, ticking budget, move-speed compensation, awake-capacity override, downed-state pheromone cleanup/special-zombie suppression, zombie need/immunity upkeep suppression, and stale zombie-count grid reset on save-load. There is still no single scenario proving the emergent zombie loop over time after save-load.
