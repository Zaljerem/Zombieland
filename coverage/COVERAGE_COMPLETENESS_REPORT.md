...
Current `scripts/coverage-inventory.sh --dependency-gates` distribution after the 2026-06-01 SoS2 target pass should be treated as a dependency triage list, not as Core behavior backlog. CE rows are no longer absent-runtime gates, CE is resolved for the current installed local build, AlienRace has moved from absent dependency to active framework plus HAR-patched Human semantics evidence, the generic ZombielandSupport contract is resolved for the local fixture, Vehicle Framework concrete movement is resolved by the local `ZLTestVehicle` fixture, and SoS2 stale-target risk is source/decompiler-aligned against the local Bqr1s checkout. The remaining external-mod queue reflects active-runtime blockers for SoS2, missing active RimConnect runtime, and standing governance/DLC-gated items rather than CE/HAR/custom-support/Vehicles startup failures. No row is currently classified as an untyped actionable gate in the active local setup.
