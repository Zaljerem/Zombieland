...
Current `scripts/coverage-inventory.sh --dependency-gates` distribution after the 2026-06-01 vehicle-fixture pass should be treated as a dependency triage list, not as Core behavior backlog. CE rows are no longer absent-runtime gates, CE is resolved for the current installed local build, AlienRace has moved from absent dependency to active framework plus HAR-patched Human semantics evidence, the generic ZombielandSupport contract is resolved for the local fixture, and the Vehicle Framework concrete movement tail is resolved by the local `ZLTestVehicle` fixture. The remaining external-mod queue reflects missing active packages for SoS2 and RimConnect plus standing governance/DLC-gated items rather than CE/HAR/custom-support/Vehicles startup failures. No row is currently classified as an untyped actionable gate in the active local setup.
