...
Current `scripts/coverage-inventory.sh --dependency-gates` distribution after the 2026-06-01 CE behavior pass: 31 rows total, split into 3 `standing-governance`, 2 `parent-branch-marker`, 2 `scenario-rollup`, 12 `dlc-content-unavailable`, 11 `external-mod-unavailable`, and 1 `removed-target-doc`. CE rows are no longer absent-runtime gates; CE is resolved for the current installed local build. The external-mod queue now reflects unavailable optional dependencies rather than CE behavior or log-clean work. No row is currently classified as an untyped actionable gate in the active local setup.
