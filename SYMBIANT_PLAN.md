...
- Current inside-symbiant high-map-pawn stress measurement on 2026-06-13 local time, MacBook Air M2 with low power mode off, save `ZL_SymbiantPawnStress_InsideSymbiant_00`, furthest vanilla zoom:
  ...
  - the stress readback now reports symbiant occupancy counts; before the timed symbiant sample, all 160 player colonists were on symbiant cells, with 2 player animals and 0 hostile humanlikes on symbiant cells,
  - 4000-cell setup used bridge-only `maxCellsOverride = 4000`, added 3999 cells for a final 4000-cell radial circle, and kept the saved gameplay setting `symbiantMaxCells = 400`,
  - no-symbiant baseline: 1734 ticks / 5005 ms, about 346 TPS,
  - 4000-cell symbiant with all colonists inside: 821 ticks / 5005 ms, about 164 TPS; this is not acceptable and needs profiling/fixing before treating the symbiant as lightweight under colonist-interaction stress,
  - both valid timed windows ended with no downed/dead player pawns and no downed/dead hostiles,
  - attempted bridge-only `noRender` and fresh `noPath` attribution probes were invalid because the stress symbiant disappeared during the short probe and free colonists dropped from 160 to 159; warning-or-higher logs still only contained the repeated hidden ritual precept warning, so the next performance step should first stabilize or bypass the stress-linked-host lifecycle before using perf profiles as evidence.
